...
namespace smabPlayground2020.Client.Pages.Boggle
{
    public partial class BoggleBoard
    {
        ...
        private readonly double DieSize = 8;
        private readonly double DieFontSize = 4.8;
        ...
        private void SelectSlot(MouseEventArgs e, BoggleSlot slot)
        {
            ...
            else if (slot.IsSelectable && (lastSlot is null || slot.AdjacentSlots.Contains(lastSlot)))
            {
    ...
    private static int ScoreWord(string word, BoggleDice.BoggleType boggleSetType)
        {
            return (boggleSetType, word.Length) switch
            {
                ...
                (BigBoggleDeluxe, 3) => 0,
                (BigBoggleOriginal, 3) => 0,
                (not BigBoggleDeluxe and not BigBoggleOriginal, 3) => 1,
                (_, 4) => 1,
                (_, 5) => 2,
                (_, 6) => 3,
                (_, 7) => 5,
